...
struct Image {
    ...
    Image(int w, int h, int c) :
        width(w), height(h), channels(c), size(w*h*c), data(new float[w*h*c]()) {}
    
    Image() :
        width(0), height(0), channels(0), size(0), data(nullptr) {}
    
    /* Only for copying contructor */
    Image(const Image& other) :
        width(other.width), height(other.height), channels(other.channels), size(other.size), data(new float[other.size]()) {
            for (int i = 0; i < size; i++)
                data[i] = other.data[i];
    }
    
    /* Only for moving contructor */
    Image(Image&& other) :
