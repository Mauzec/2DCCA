...
std::vector<Keypoint> find_keypoints_and_descriptors(const Image& img, bool add_to_kps = false, string addition = "", float sigma_min=SIGMA_MIN,
...
std::vector<Keypoint> find_keypoints_and_descriptors(const Image& img, bool add_to_kps, string addition, float sigma_min,
                                                     int num_octaves, int scales_per_octave,
                                                     float contrast_thresh, float edge_thresh,
                                                     float lambda_ori, float lambda_desc) {
    ...
    const char* json_path = (add_to_kps) ? addition.c_str() : "kps.json";
    std::vector<Keypoint> kps;
    ofstream file(json_path);
    json jn;
    jn["kpsdes"] = json::array();
    for (Keypoint& kp_tmp : tmp_kps) {
        ...
        for (float theta : orientations) {
            ...
            kps.push_back(kp);
            jn["kpsdes"].push_back({{"x", kp.x}, {"y", kp.y}, {"des", kp.descriptor}});
        }
    }
    file << setw(4) << jn << endl;
    file.close();
